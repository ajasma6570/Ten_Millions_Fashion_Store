    ...
    dashboard:async(req,res)=>{
        try{
         ...
         const current=  await orderModel.aggregate([
            ...
            res.status(200).render('dashboard',{revenue:TotalSales,order:ordercount,product:productcount,category:categorycount,orders:orders,current:current,monthlysale:monthly})
        }catch(error){
            res.status(404).render('error',{error:error.message})        }
    }
    ,
        ...
        try{
            ...
            if(admindata){
                if(admindata.password===req.body.adminPassword ){
                    ...
                    const details= await orderModel.aggregate([
                        {$match: {paymentStatus: "Paid"}},{$group: {_id: null,totalSales: { $sum: "$totalAmount" }}}])
                    ...
                    // res.status(200).render('dashboard',{revenue:TotalSales,order:ordercount,product:productcount,category:categorycount})
                }else{
...
//***********************************Product COntroller******************************************** 
    ...
    productList:async(req,res)=>{
        try{
            ...
            res.status(200).render('products',{product:products,notice:notice[0]||""})
        }catch(error){
            res.status(404).render('error',{error:error.message})            }
    }, 
    ...
    addProductpage:async(req,res)=>{
        try{
            ...
            const notice= req.flash('notice')                
            res.status(200).render('addproduct',{category:categorylist,notice:notice[0]||""})
        }catch(error){
            res.status(404).render('error',{error:error.message})        }
    ...
    editproduct:async(req,res)=>{
        try{
                ...
                const categorylist= await category.find({list:{$ne:1}})
                res.status(200).render('editproduct',{productdata:productdata,category:categorylist,activeValue:productdata.category})
        }catch(error){
            res.status(404).render('error',{error:error.message})            }
    },
...
//***********************************Category controller******************************************** 
        ...
        Categories:async(req,res)=>{
            try{
                ...
                res.status(200).render('Categories',{category:categorylist,notice:noticed[0],icon:icons[0]}) 
            }catch(error){
                res.status(404).render('error',{error:error.message})            }
        },
        ...
        addingCategory:async(req,res)=>{
            try{
                ...
                const existcategory= await category.findOne({categoryLower:categorylower})
                if(existcategory){
        ...
        listCategories:async(req,res)=>{
            try{
                ...
                if(categorydata.list===0){
                    const user=await category.updateOne({_id:categoryId},{$set:{list:1}})
                    req.flash("notice","Category unlisted");
                    req.flash("icon","error");
                    res.status(200).redirect('/admin/Categories')
                }else{
                    console.log(categorydata.list===0);
                    const user=await category.updateOne({_id:categoryId},{$set:{list:0}})
        ...
        addcategories:async(req,res)=>{
            try{
                const notice =req.flash("notice")
                res.status(200).render('addcategory',{notice:notice[0],icon:"error"})
            }catch(error){
                res.status(404).render('error',{error:error.message})            }
        },
...
//***********************************logout controller******************************************** 
   ...
   categorywise:async(req,res)=> {
                    const orders = await orderModel.find({ status: "Delivered" }).populate("products.productid");
            ...
            res.status(200).json({data:datasets,date:months,categoryname:categoryNames})
        },
        ...
        approveReturn: async (req, res) => {
          ...
          categoryOffer:async(req,res)=>{
            ...
            res.render('categoryoffer',{categories,offered,successmsg,messageAlert})
          },
                      ...
                      $set: {
                        ...
                        categoryprice: {
                          $trunc: {
                            $subtract: [
                              '$price',
                              { $multiply: ['$price', { $divide: [offerpercentageNumeric, 100] }] }
                            ]
                          }
                        }
                      }
